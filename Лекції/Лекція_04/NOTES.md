...
### 19. WireGuard: революція у VPN-протоколах

WireGuard з'явився у 2015 році як дипломна робота Джейсона Деллера (Jason Donenfeld) і перевернув усталені уявлення про те, яким має бути VPN-протокол. Якщо OpenVPN налічує сотні тисяч рядків коду, а IPsec-стек Linux — ще більше, то WireGuard спроектований з принципом мінімалізму: перша версія коду налічувала близько 4000 рядків — стільки, що один безпековий дослідник може перевірити весь протокол за кілька днів.

**Архітектурні рішення** WireGuard докорінно відрізняються від попередників. По-перше, **фіксований криптографічний стек**: немає вибору алгоритмів, cipher suites чи режимів — лише Curve25519 для обміну ключами, ChaCha20-Poly1305 для AEAD-шифрування, BLAKE2s для хешування та MAC і SipHash24 для хешування таблиць. Це усуває цілий клас вразливостей, пов'язаних з погано узгодженими криптографічними параметрами (downgrade attacks, weak cipher negotiation). По-друге, **kernel-space обробка**: з Linux 5.6 WireGuard є частиною ядра, що означає обробку пакетів без переключення контекстів між user і kernel space — звідси і рекордна продуктивність (1-5 Гбіт/с на одному ядрі з AES-NI). По-третє, **UDP-only транспорт**: WireGuard відмовився від підтримки TCP, оскільки TCP-over-TCP (TCP-всередині-TCP-тунелю) має катастрофічні проблеми з продуктивністю через подвійне керування потоком.

**Cryptokey Routing** — унікальна концепція WireGuard. Кожному peer'у відповідає набір дозволених IP-адрес (`AllowedIPs`). При відправці пакету WireGuard знаходить peer з IP-адресою призначення у своїй таблиці `AllowedIPs` і шифрує пакет публічним ключем цього peer'а. При отриманні пакету WireGuard перевіряє, чи IP-адреса відправника відповідає записаному для цього peer'а `AllowedIPs` — якщо ні, пакет відкидається. Це суміщає функції routing та access control в одному механізмі.

**«Stealth»-властивість WireGuard** — ще одна принципова відмінність. WireGuard не відповідає на пакети від незнайомих відправників: якщо на UDP-порт WireGuard надійде звичайний UDP-пакет від непізнаного source, WireGuard просто відкине його без жодної відповіді, не виявляючи навіть факту свого існування. Це ускладнює виявлення VPN-шлюзу під час розвідки мережі та надає природний захист від DoS-атак.

### 20. MACsec: шифрування на канальному рівні

**MACsec (IEEE 802.1AE)** — стандарт шифрування для Ethernet на рівні L2. На відміну від IPsec або WireGuard, що шифрують IP-пакети, MACsec шифрує **Ethernet-кадри** між двома точками одного канального сегменту: між хостом та комутатором або між двома комутаторами у дата-центрі.

MACsec додає **SecTAG** (безпековий тег) між MAC-адресами та EtherType полем вихідного кадру та **ICV** (Integrity Check Value) в кінці кадру. SecTAG включає 64-бітний Packet Number для захисту від replay-атак. Шифрування виконується алгоритмом AES-128-GCM або AES-256-GCM. Управління ключами здійснюється протоколом **MKA (MACsec Key Agreement, IEEE 802.1X)**, що забезпечує автентифікацію учасників та узгодження ключів.

Практичне застосування MACsec — захист фізичних каналів між комутаторами ЦОД або між комутатором та сервером. У хмарних середовищах AWS, Azure та GCP використовують MACsec для захисту виділених фізичних підключень (AWS Direct Connect, Azure ExpressRoute, GCP Cloud Interconnect), що є набагато кращою альтернативою передачі трафіку в незашифрованому вигляді через орендований волоконно-оптичний канал. Overhead MACsec становить лише 32 байти на кадр, порівняно з 50-70 байтами у IPsec, що робить його більш ефективним для high-throughput сценаріїв усередині ЦОД.

### 21. DTLS та QUIC: VPN-протоколи на UDP основі

**DTLS (Datagram TLS, RFC 6347)** вирішує проблему TLS over TCP: TLS надає надійний захищений транспорт, але вимагає TCP, що вносить затримку та проблеми head-of-line blocking. DTLS переносить TLS-механізм на UDP, зберігаючи автентифікацію та шифрування, але без гарантій доставки та порядку.

У VPN-контексті DTLS використовується у Cisco AnyConnect (клієнт підключається через TLS для управляючого каналу, але переключається на DTLS для даних), WebRTC DTLS-SRTP (безпека медіапотоків у браузерних відеодзвінках), та деяких реалізаціях OpenVPN (experimental DTLS mode). Ключова перевага: при втраті пакету наступний пакет продовжує оброблятися без затримки, характерної для TCP-retransmission. Це критично для VoIP та відеоконференцій через VPN.

**QUIC (RFC 9000)** — принципово новий транспортний протокол від Google (тепер стандартизований IETF), що поєднує TLS 1.3, мультиплексування потоків та connection migration у єдиному UDP-базованому протоколі. QUIC усуває head-of-line blocking TCP, підтримує 0-RTT відновлення з'єднань та може мігрувати між мережами (Wi-Fi → 4G) без повторного рукостискання. HTTP/3 базується на QUIC.

**MASQUE (Multiplexed Application Substrate over QUIC Encryption, RFC 9298)** — новий стандарт тунелювання через QUIC. Cloudflare WARP та Apple Private Relay вже використовують MASQUE для підключення клієнтів. MASQUE передає IP-пакети або UDP-потоки через QUIC-з'єднання, що дозволяє обходити мережеве обладнання, яке блокує нестандартні UDP-порти (QUIC зазвичай використовує порт 443, де блокування викличе проблеми для всього HTTPS-трафіку H/3). З точки зору клієнта MASQUE-тунель виглядає як звичайне HTTPS-з'єднання до вебсайту.

### 22. Proprietary VPN протоколи та їх розвиток

Поряд зі стандартизованими протоколами (IPsec, OpenVPN, WireGuard) на ринку присутні численні пропрієтарні рішення, що використовують власні протоколи або значно модифіковані версії стандартних.

**Cisco AnyConnect DTLS** — один з найпоширеніших корпоративних VPN-клієнтів. Використовує TLS для управляючого каналу та DTLS для даних. Ключова функція — Always-On VPN із перевіркою стану пристрою (posture check): перед наданням доступу клієнт перевіряє наявність актуального антивіруса, firewall, шифрування диска та версії ОС. Cisco Secure Client (нова назва AnyConnect) інтегрується з Cisco Identity Services Engine (ISE) для мікросегментації та контролю доступу на рівні мережі.

**ExpressVPN Lightway** — протокол, відкритий у 2021 році, базується на бібліотеці wolfSSL. Lightway використовує UDP за замовчуванням (з fallback на TCP), підтримує 0-RTT відновлення з'єднань, що робить перемикання між мережами майже миттєвим. Особливістю є суттєво менший код порівняно з OpenVPN, що дозволяє проводити більш повний аудит безпеки.

**NordVPN NordLynx** — це WireGuard з додатковим шаром подвійного NAT для вирішення проблеми конфіденційності WireGuard. Стандартний WireGuard зберігає IP-адреси peer'ів у пам'яті постійно, що суперечить принципам no-logs VPN. NordLynx додає внутрішній тунель між клієнтом та сервером, де зовнішній сервер знає лише тимчасово призначений внутрішній IP, а не реальний IP клієнта.

### 23. Вектори атак на VPN-тунелі та захист

Розуміння типових векторів атак на VPN-інфраструктуру є необхідним для її коректного захисту. Атаки на VPN поділяються на кілька категорій залежно від цілі: самого протоколу, реалізації, конфігурації або облікових даних.

**Протокольні атаки** спрямовані на слабкості у самому криптографічному протоколі. IKEv1 Aggressive Mode з PSK є класичним прикладом: зловмисник може захопити хеш PSK та підібрати його офлайн за допомогою GPU-кластеру. Атака SWEET32 на 3DES дозволяла відновлювати дані при достатньому обсязі трафіку. SSL POODLE та BEAST атаки використовували CBC-режим у старих версіях SSL/TLS.

**Атаки на реалізацію** використовують помилки у конкретному ПЗ, а не у протоколі. Масштабні злами Ivanti (Pulse Secure) у 2023-2024 роках використовували уразливості типу Authentication Bypass та Remote Code Execution. Фортунатно, WireGuard з його мінімалістичним кодом та kernel-space реалізацією має суттєво меншу поверхню атаки.

**Атаки на конфігурацію** — найпоширеніший вектор: слабкий PSK, дефолтні облікові дані, ненастроєний firewall перед VPN, відсутність MFA, занадто широкі права доступу. Кожна з цих помилок може дозволити зловмиснику отримати доступ до корпоративної мережі навіть без злому самого VPN-протоколу.

**Захист** включає: регулярне оновлення ПЗ (більшість успішних зломів використовують відомі, але незаплатані вразливості), MFA обов'язково, мінімізацію криптографічних можливостей (вимкнення застарілих алгоритмів), firewall перед VPN-шлюзом (rate limiting IKE, whitelist IP), моніторинг та alerting через SIEM, а також регулярне тестування безпеки (penetration testing VPN-шлюзу).

### 24. Практичні рекомендації вибору VPN-протоколу

При виборі VPN-протоколу для конкретного сценарію необхідно враховувати кілька ключових параметрів: вимоги до сумісності з клієнтськими пристроями, обмеження мережевого середовища (наявність NAT, firewall-правила), вимоги до безпеки та продуктивності, а також можливості операційної підтримки.

**Для нових корпоративних розгортань** Remote Access VPN оптимальним вибором є **IKEv2/IPsec** (максимальна сумісність з Windows, macOS, iOS, Android без додаткових клієнтів) або **WireGuard** (максимальна продуктивність, простота управління, ідеально для Linux-centric середовищ). Якщо потрібен обхід рестриктивних корпоративних firewall, що блокують нестандартні UDP-порти, — **OpenVPN TCP 443** або **SSTP** є кращим вибором.

**Для Site-to-Site** з'єднань між офісами: IKEv2/IPsec з BGP-маршрутизацією через VTI-інтерфейси для динамічного маршрутизатора; WireGuard для простих конфігурацій між Linux-шлюзами; GRE/IPsec для сумісності з Cisco та іншим enterprise-обладнанням.

**Для мобільних клієнтів** та сценаріїв з частою зміною мереж (Wi-Fi ↔ 4G): IKEv2 з MOBIKE забезпечує seamless failover; WireGuard за замовчуванням підтримує роумінг через статичні IP у конфігурації.

**Для legacy** пристроїв без підтримки сучасних протоколів: L2TP/IPsec підтримується навіть у Windows XP та старих Android пристроях, хоча цей вибір рекомендується лише для тимчасових рішень.

Важливо пам'ятати: навіть найкращий протокол не компенсує погану конфігурацію безпеки. MFA, правильна ієрархія CA, принцип мінімальних привілеїв та регулярне оновлення ПЗ — ці організаційні заходи часто мають більший вплив на безпеку, ніж вибір конкретного протоколу.
